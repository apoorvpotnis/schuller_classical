...
\begin{document}
    ...
    \begin{figure}[htb]
        \centering
        \subcaptionbox{\(U \in \topology_{\symrm{s}}\) is an open set of \(\rr^2\). The dashed line indicates that the points on the line are not contained in \(U\). For all \(a \in U\), we can always find an \(r > 0\) such that \(\ball_r(a)\) in fully contained in \(U\). \label{fig:openset}}[0.48\textwidth]
        {
            \begin{tikzpicture}[scale=0.7, use Hobby shortcut]
                \draw[std line width, smooth, closed, dashed] (0,0) .. ++ (1,0) .. ++ (2,1) .. ++ (-3,3) .. ++ (-2, -1) -- cycle;
                \node at (3.15,3.1) {\(U\)};
               \draw[std line width, red, dashed] (1,1.5) circle[radius=0.8] node[above, yshift=15] {\(\ball_r (a)\)};
               \fill (1,1.5) circle[radius=0.05] node[right] {\(a\)};
            \end{tikzpicture}
        }
        \qquad
        \subcaptionbox{\(V \ni \topology_{\symrm{s}}\) is a closed set of \(\rr^2\). The solid line used to depict \(V\) indicates that the points on the line belong to \(V\). For all \(a' \in \partial V\), we can never find an \(r > 0\) such that \(\ball_r(a)\) in fully contained in \(V\). Here, \(\partial V\) denotes the boundary of \(V\). \label{fig:closed}}[0.48\textwidth]
        {
            \begin{tikzpicture}[scale=0.7, use Hobby shortcut]
                \draw[std line width, smooth, closed] (0,0) .. ++ (1,0) .. ++ (2,1) .. ++ (-3,3) .. ++ (-2, -1) -- cycle;
                \node at (3.15,3.1) {\(V\)};
               \draw[std line width, red, dashed] (3,1) circle[radius=0.8] node[above, yshift=12, xshift=-20] {\(\ball_r (a')\)};
               \fill (3,1) circle[radius=0.05] node[right] {\(a'\)};
            \end{tikzpicture}
        }
