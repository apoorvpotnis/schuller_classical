...
\begin{luacode}
    local kerns = {
        ["NewCMMath-Book"] = {
          ["uni211D.bb"] = { topright = { { kern = -70 } } } -- double-struck capital R
        },
        ["NewCMMath-Bold"] = {
          ["uni211D.bb"] = { topright = { { kern = -30 } } } -- double-struck capital R
        }
    }
    local function addkerns(tfmdata)
        if not tfmdata.mathparameters then return end

        local font_kerns = kerns[tfmdata.properties.fontname]
        if not font_kerns then return end

        local unicodes   = tfmdata.resources.unicodes
        local characters = tfmdata.characters

        for name, specification in pairs(font_kerns) do
            local unicode = unicodes[name]
            local character = unicode and characters[unicode]

            if character then
                character.mathkerns = character.mathkerns or {}

                for side, data in pairs(specification) do
                    character.mathkerns[side] = data
                end
            end
        end
    end
...
\the\everymath%
\Umathsubshiftdown\textstyle=1.8pt\Umathsubshiftdrop\textstyle=0.5pt}
\everydisplay=\expandafter{%
\the\everydisplay%
\Umathsubshiftdown\displaystyle=1.8pt\Umathsubshiftdrop\displaystyle=0.5pt}
...
\begin{filecontents}[overwrite]{schuller_classical.bib}
    ...
    @book{Moretti,
        ...
        subtitle = {Classical, Lagrangian and Hamiltonian Mechanics, Stability Theory, Special Relativity},
        year = {2023},
        volume = {50},
        translator = {Simon G.\ Choissi},
        origlanguage = {fromitalian}
    }
...
% moewe's code from here https://tex.stackexchange.com/a/609093/128462
...
\newcommand{\ltwo}{\symrm{L\kern-0.4pt^2}}
...
\newcommand{\hilbert}{\mathchoice
  {\raisebox{0.4pt}{$\displaystyle\symcal{H}$}}
  {\raisebox{0.4pt}{$\textstyle\symcal{H}$}}
  {\raisebox{0pt}{$\scriptstyle\symcal{H}$}}
  {\raisebox{0pt}{$\scriptscriptstyle\symcal{H}$}}
}
...
% from the amsmath package https://tex.stackexchange.com/a/37791/128462 and https://tex.stackexchange.com/a/685895/128462

\renewcommand{\colon}{\nobreak\mskip2mu\mathpunct{}\nonscript
  \mkern-\thinmuskip{\Umathchar"6 "0 "3A}\mskip6muplus1mu\relax}

% code from egreg's answer https://tex.stackexchange.com/a/757762/128462
