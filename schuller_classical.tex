...
\begin{document}
	...
	Suppose we have an atlas of the world with maps of different regions on different pages. We refer to these maps as charts. These charts are flat, but if we join the charts such that regions common to two charts overlap on each other, we unsurprisingly get the globe of the earth. The global, spherical structure of earth is encoded in the overlapping regions of the charts. This idea shall be used in the study of manifolds.
	...
	\begin{figure}[htb]
		...
		\begin{subfigure}[b]{0.41\textwidth}
			...
			\label{fig:paralleltransportb}
		\end{subfigure}
		\captionof{figure}{Parallel transport of vectors}
	...
	\begin{figure}[htb]
		...
		\begin{tikzpicture}
			...
			\draw[gray] (0, -\h) ellipse [x radius = \a, y radius = \b];
			\draw[smooth, thick] (-0.3, 1) .. controls (-0.2,0) and (0.2,-0) .. (0.3,-1);
		\end{tikzpicture}
		\caption{Light cone}
		\label{fig:lightcone}
	\end{figure}
