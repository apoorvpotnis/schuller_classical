...
%           \AtPageLowerLeft{
    %           \hspace*{1in}
    %           \hspace{2cm}\color{blueone}\rule{2pt}{\paperheight}
%           }
%       }
        \begin{tikzpicture}[remember picture, overlay]
            \draw[blueone, line width=4pt] (1.5cm,-\paperheight) -- (1.5cm,\paperheight);
            \draw[blueone, line width=4pt] ([yshift=6cm]current page.west) -- ([yshift=6cm]current page.east);
        \end{tikzpicture}
    ...
    \begin{figure}[htb]
        \centering
        \subcaptionbox{\(U \in \topology_{\symrm{s}}\) is an open set of \(\rr^2\). The dashed line indicates that the points on the line are not contained in \(U\). For all \(a \in U\), we can always find an \(r > 0\) such that \(\ball_r(a)\) in fully contained in \(U\). \label{fig:openset}}[0.44\textwidth]
        {
        ...
        \qquad
        \subcaptionbox{\(V \notin \topology_{\symrm{s}}\) is a closed set of \(\rr^2\). The solid line used to depict \(V\) indicates that the points on the line belong to \(V\). For all \(a' \in \partial V\), we can never find an \(r > 0\) such that \(\ball_r(a)\) in fully contained in \(V\). Here, \(\partial V\) denotes the boundary of \(V\). \label{fig:closed}}[0.44\textwidth]
        {
            \begin{tikzpicture}[scale=0.7, use Hobby shortcut]
