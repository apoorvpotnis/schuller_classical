...
\begin{document}
    ...
    \begin{figure}[htb]
        \centering
        \subcaptionbox{\(U \in \topology_{\symrm{s}}(\rr^2)\) is an open set of \(\rr^2\). The dashed line indicates that the points on the line are not contained in \(U\). For all \(a \in U\), we can always find an \(r > 0\) such that \(\ball_r(a)\) in fully contained in \(U\). \label{fig:openset}}[0.45\textwidth]
        {
            \begin{tikzpicture}[scale=0.7, use Hobby shortcut]
                % \draw[std line width, smooth, closed, dashed] (0,0) .. ++ (1,0) .. ++ (2,1) .. ++ (-3,3) .. ++ (-2, -1) -- cycle;
                \draw[std line width, smooth, dashed] (0,0) .. ++ (1,0) .. ++ (2,1);
                \node at (3.15,3.1) {\(U\)};
        ...
        \hfill
        \subcaptionbox{\(V \notin \topology_{\symrm{s}}(\rr^2)\) is a closed set of \(\rr^2\). The solid line used to depict \(V\) indicates that the points on the line belong to \(V\). For all \(a' \in \partial\kern-1pt V\), we can never find an \(r > 0\) such that \(\ball_r(a)\) in fully contained in \(V\). Here, \(\partial\kern-1pt V\) denotes the boundary of \(V\).\label{fig:closed}}[0.45\textwidth]
        {
            \begin{tikzpicture}[scale=0.7, use Hobby shortcut]
