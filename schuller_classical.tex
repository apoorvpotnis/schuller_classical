...
\begin{document}
    ...
    \begin{figure}[htb]
        ...
        \hfill
        \subcaptionbox{\(V \notin \topology_{\symrm{s}}(\rr^2)\) is a closed set of \(\rr^2\). The solid line used to depict \(V\) indicates that the points on the line belong to \(V\). For all \(a' \in \partial V\), we can never find an \(r > 0\) such that \(\ball_r(a)\) in fully contained in \(V\). Here, \(\partial V\) denotes the boundary of \(V\).\label{fig:closed}}[0.45\textwidth]
        {
            \begin{tikzpicture}[scale=0.7, use Hobby shortcut]
